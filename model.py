...
class MacridVAE(nn.Module):
    ...
    def load(cls, model_file):
        ...
        model = MacridVAE(num_items=config_dict['num_items'],
...
class RegularizationMode(enum.Enum):
    DISTANCE = 'distance'
    MUTUAL_INFORMATION = 'mutual_info'


class ADDVAE(nn.Module):
    def __init__(self,
                 num_items: int,
                 num_words: int,
                 layers: list,
                 ...
                 cmode: str = 'regularization',
                 rmode: str = 'mutual_info',
                 coda: bool = False,
                 scale: bool = True,
                 subtract_mean: bool = False,
                 subtract_mean_n: bool = False):
        super(ADDVAE, self).__init__()
        if reg_weights is None:
            reg_weights = [0.0, 0.0]
        self.num_items = num_items
        ...
        self.subtract_mean_n = subtract_mean_n

        self.cf_network = MacridVAE(num_items=self.num_items,
                                    layers=self.layers,
                                    emb_size=self.emb_size,
                                    dropout=self.dropout,
                                    num_prototypes=self.num_prototypes,
        ...
        self.text_network = MacridVAE(num_items=self.num_words,
                                      layers=self.layers,
                                      emb_size=self.emb_size,
                                      dropout=self.dropout,
                                      num_prototypes=self.num_prototypes,
    ...
    def forward(self, input_rating, input_text, need_prob_k=False):
        ...
        if self.cmode == CombinationMode.ADDITION.value:
            z_list = [cf_z_list[i] + text_z_list[i] for i in range(len(cf_z_list))]
            reg_term = self.regularization_term(cf_z_list, text_z_list)
        else:
            ...
            reg_term = self.regularization_term(cf_z_list_given_text, text_z_list_given_cf)
    ...
    @staticmethod
    def l2_distance(x1: Tensor, x2: Tensor):
        return ((x1 - x2) ** 2).sum(dim=-1)

    def l2_distance_reg(self, list_a: List[Tensor], list_b: List[Tensor]):
        assert len(list_a) == len(list_b)
        reg_terms = [self.l2_distance(list_a[i], list_b[i]) for i in range(len(list_a))]
        return torch.mean(torch.cat(reg_terms))
    ...
    def attention_layer(self, q_mat, k_mat, v_mat):
        if self.coda:
            if self.scale:
                p_1 = q_mat.bmm(k_mat.transpose(1, 2)) / np.sqrt(self.emb_size)
                if self.subtract_mean_n:
                    p_1 = (p_1 - p_1.mean(dim=-1, keepdims=True))
                p_1 = p_1.tanh()
                p_2 = -(q_mat.unsqueeze(dim=2) - k_mat.unsqueeze(dim=1)).abs().sum(dim=-1) / np.sqrt(self.emb_size)
            else:
        ...
        return (q_mat.bmm(k_mat.transpose(1, 2)) / np.sqrt(self.emb_size)).softmax(dim=-1).bmm(v_mat)
    ...
    def regularization_term(self, cf_z_list, text_z_list):
        if self.rmode == RegularizationMode.DISTANCE.value:
            reg_term = self.l2_distance_reg(cf_z_list, text_z_list)
        elif self.rmode == RegularizationMode.MUTUAL_INFORMATION.value:
            reg_term = self.mutual_information_reg(cf_z_list, text_z_list)
        else:
            raise Exception
        return reg_term

    def calculate_loss(self, input_rating, input_text):
        if self.cmode == CombinationMode.REGULARIZATION.value:
            cf_loss, cf_z_list = self.cf_network.calculate_loss(input_rating, need_z=True)
            text_loss, text_z_list = self.text_network.calculate_loss(input_text, need_z=True)
            final_loss = cf_loss + self.lambda_text * text_loss
            if self.lambda_reg > 0:
                reg_term = self.regularization_term(cf_z_list, text_z_list)
                final_loss = final_loss + self.lambda_reg * reg_term
            return final_loss
        elif self.cmode == CombinationMode.ADDITION.value or \
                self.cmode == CombinationMode.ATTENTION.value:
    ...
    def predict(self, input_rating, input_text):
        if self.cmode == CombinationMode.REGULARIZATION.value:
            rating_prediction = self.cf_network.predict(input_rating)
            text_prediction = self.text_network.predict(input_text)
        elif self.cmode == CombinationMode.ADDITION.value or \
                self.cmode == CombinationMode.ATTENTION.value:
            rating_prediction, text_prediction, _, _, _ = \
                self.forward(input_rating, input_text)
        else:
            raise Exception
    ...
    def predict_per_prototype_output(self, input_rating, input_text):
        if self.cmode == CombinationMode.REGULARIZATION.value:
            rating_prob_k_list = \
                self.cf_network.predict_per_prototype_outputs(input_rating)
            text_prob_k_list = \
                self.text_network.predict_per_prototype_outputs(input_text)
            return rating_prob_k_list, text_prob_k_list
        elif self.cmode == CombinationMode.ADDITION.value or \
                self.cmode == CombinationMode.ATTENTION.value:
            _, _, _, _, _, rating_prob_k_list, text_prob_k_list = \
                self.forward(input_rating, input_text, need_prob_k=True)
        else:
            raise Exception
        return rating_prob_k_list, text_prob_k_list
    ...
    def save(self, out_file):
        config_dict = {
            ...
            'subtract_mean_n': self.subtract_mean_n,
            'state_dict': self.state_dict()
        }
        torch.save(config_dict, out_file)
        print(f'Save model to {out_file}')
    ...
    def load(cls, model_file):
        config_dict = torch.load(model_file)
        model = ADDVAE(num_items=config_dict['num_items'],
                       ...
                       cmode=config_dict['cmode'],
                       rmode=config_dict['rmode'],
                       coda=config_dict['coda'],
                       scale=config_dict['scale'],
                       subtract_mean=config_dict['subtract_mean'],
                       subtract_mean_n=config_dict['subtract_mean_n'])
        model.load_state_dict(config_dict['state_dict'])
        return model
